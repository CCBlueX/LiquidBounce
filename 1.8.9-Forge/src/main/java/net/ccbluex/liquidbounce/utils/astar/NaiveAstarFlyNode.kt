...
class NaiveAstarFlyNode(val x: Int, val y: Int, val z:Int, var parent_node: NaiveAstarFlyNode? = null) : AstarNode
{
    ...
    {
        parent_node = p as NaiveAstarFlyNode?
    }
    ...
    {
        val node = begin as NaiveAstarFlyNode
    ...
    {
        val node = end as NaiveAstarFlyNode
        return (abs(node.x - x) + abs(node.y - y) + abs(node.z - z)).toDouble()
    }
