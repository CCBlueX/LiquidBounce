...
data class Rotation(var yaw: Float, var pitch: Float) {
    ...
    fun applyStrafeToPlayer(event: StrafeEvent) {
        val dif = ((MathHelper.wrapAngleTo180_float(MinecraftInstance.mc.thePlayer.rotationYaw - this.yaw
                - 23.5f - 135)
                + 180) / 45).toInt()
        ...
        if (d >= 1.0E-4f) {
            ...
            MinecraftInstance.mc.thePlayer.motionX += calcStrafe * yawCos - calcForward * yawSin.toDouble()
            MinecraftInstance.mc.thePlayer.motionZ += calcForward * yawCos + calcStrafe * yawSin.toDouble()
        }
    }
}
