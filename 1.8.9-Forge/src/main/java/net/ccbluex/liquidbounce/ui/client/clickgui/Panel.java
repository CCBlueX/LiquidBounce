...
public abstract class Panel extends MinecraftInstance {
    ...
    public void drawScreen(int mouseX, int mouseY, float button) {
        ...
        final int maxElements = ((ClickGUI) Objects.requireNonNull(ModuleManager.getModule(ClickGUI.class))).maxElementsValue.get();
    ...
    private int getElementsHeight() {
        ...
        int count = 0;
        for(final Element element : elements) {
            if(count >= ((ClickGUI) Objects.requireNonNull(ModuleManager.getModule(ClickGUI.class))).maxElementsValue.get())
                continue;
