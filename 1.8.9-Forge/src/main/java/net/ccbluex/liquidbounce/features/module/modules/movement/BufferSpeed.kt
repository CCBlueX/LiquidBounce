    ...
    private val isNearBlock: Boolean
        get() {
            ...
            for (blockPos in blocks) if (theWorld!!.getBlockState(blockPos).block.blockBoundsMaxY ==
                    ...
                    !theWorld.getBlockState(blockPos).block.translucent && theWorld.getBlockState(blockPos).block == classProvider.getBlockEnum(BlockType.WATER) &&
                    !classProvider.isBlockSlab(theWorld.getBlockState(blockPos).block) || theWorld.getBlockState(blockPos).block == classProvider.getBlockEnum(BlockType.BARRIER)) return true
            return false
        }
}