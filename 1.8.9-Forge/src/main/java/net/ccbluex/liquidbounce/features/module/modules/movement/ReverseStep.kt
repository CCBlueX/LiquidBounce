...
class ReverseStep : Module() {
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (collideBlock(thePlayer.entityBoundingBox, object : Collidable {
                    override fun collideBlock(block: IBlock?): Boolean {
                        return classProvider.isBlockLiquid(block)
                    }
                }) || collideBlock(classProvider.createAxisAlignedBB(thePlayer.entityBoundingBox.maxX, thePlayer.entityBoundingBox.maxY, thePlayer.entityBoundingBox.maxZ, thePlayer.entityBoundingBox.minX, thePlayer.entityBoundingBox.minY - 0.01, thePlayer.entityBoundingBox.minZ), object : Collidable {
                    override fun collideBlock(block: IBlock?): Boolean {
                        return classProvider.isBlockLiquid(block)
                    }
                })) return
