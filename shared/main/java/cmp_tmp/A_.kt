...
// UPDATE EVENTS
        ...
        if (!eagleValue.get().equals("Off", true) && !shouldGoDown)
        {
            ...
            if (edgeDistanceValue.get() > 0 && !shouldGoDown)
            {
                for (facingType in EnumFacingType.values())
                {
                    ...
                    if (!side.isNorth() && !side.isEast() && !side.isSouth() && !side.isWest()) continue
                    val blockPosition = WBlockPos(mc.thePlayer!!.posX, mc.thePlayer!!.posY - 1.0, mc.thePlayer!!.posZ)
            ...
            {
                val shouldEagle: Boolean = mc.theWorld!!.getBlockState(WBlockPos(mc.thePlayer!!.posX, mc.thePlayer!!.posY - 1.0, mc.thePlayer!!.posZ)).block == (classProvider.getBlockEnum(BlockType.AIR)) || dif < edgeDistanceValue.get()
    ...
    {
        if (strafeMode.get().equals("Off", true)) return
        ...
        if (rotationsValue.get() && (keepRotationValue.get() || !lockRotationTimer.hasTimePassed(keepLengthValue.get())) && lockRotation != null)
        {
            if (targetPlace == null)
            {
                ...
                for (i in 0..7)
                {
                    if (abs(RotationUtils.getAngleDifference(lockRotation!!.yaw, (i * 45).toFloat())) < abs(RotationUtils.getAngleDifference(lockRotation!!.yaw, yaw)))
                    {
    ...
    fun onMotion(event: MotionEvent)
    {
        ...
        {
            setRotation(lockRotation!!)
            if (eventState == EventState.POST) lockRotationTimer.update()
        }

        // Face block
        if ((facesBlock || !rotationsValue.get()) && placeModeValue.get().equals(eventState.stateName, true)) place()

        // Update and search for a new block
        if (eventState == EventState.PRE && strafeMode.get().equals("Off", true)) update()

        // Reset placeable delay
        if (targetPlace == null && placeDelay.get()) delayTimer.reset()
    }
    ...
    {
        val blockPosition: WBlockPos = if (shouldGoDown) (if (mc.thePlayer!!.posY == mc.thePlayer!!.posY.toInt() + 0.5) WBlockPos(mc.thePlayer!!.posX, mc.thePlayer!!.posY - 0.6, mc.thePlayer!!.posZ)
        else WBlockPos(mc.thePlayer!!.posX, mc.thePlayer!!.posY - 0.6, mc.thePlayer!!.posZ).down())
        else (if (sameYValue.get() && launchY <= mc.thePlayer!!.posY) WBlockPos(mc.thePlayer!!.posX, launchY - 1.0, mc.thePlayer!!.posZ)
        else (if (mc.thePlayer!!.posY == mc.thePlayer!!.posY.toInt() + 0.5) WBlockPos(mc.thePlayer!!)
        ...
        if (expand)
        {
            ...
            {
                if (search(blockPosition.add(when (mc.thePlayer!!.horizontalFacing)
                    {
                        classProvider.getEnumFacing(EnumFacingType.WEST) -> -i
                        classProvider.getEnumFacing(EnumFacingType.EAST) -> i
                        else -> 0
                    }, 0, when (mc.thePlayer!!.horizontalFacing)
                    {
...
// SCAFFOLD VISUALS
    ...
    fun onRender3D(event: Render3DEvent)
    {
        ...
        {
            val blockPos = WBlockPos(mc.thePlayer!!.posX + when (mc.thePlayer!!.horizontalFacing)
            {
                classProvider.getEnumFacing(EnumFacingType.WEST) -> -i.toDouble()
                classProvider.getEnumFacing(EnumFacingType.EAST) -> i.toDouble()
                else -> 0.0
            }, if (sameYValue.get() && launchY <= mc.thePlayer!!.posY) launchY - 1.0 else mc.thePlayer!!.posY - (if (mc.thePlayer!!.posY == mc.thePlayer!!.posY + 0.5) 0.0 else 1.0) - if (shouldGoDown) 1.0 else 0.0, mc.thePlayer!!.posZ + when (mc.thePlayer!!.horizontalFacing)
            {
            ...
            while (xSearch <= if (auto) 0.9 else 0.5 + xzRV / 2)
            {
                ...
                while (ySearch <= if (auto) 0.9 else 0.5 + yRV / 2)
                {
                    ...
                    while (zSearch <= if (auto) 0.9 else 0.5 + xzRV / 2)
                    {
                        ...
                        val obj = mc.theWorld!!.rayTraceBlocks(eyesPos, vector, stopOnLiquid = false, ignoreBlockWithoutBoundingBox = false, returnLastUncollidableBlock = true)
