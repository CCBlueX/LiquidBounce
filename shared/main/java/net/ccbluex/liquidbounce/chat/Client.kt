...
abstract class Client : ClientListener, MinecraftInstance()
{
    ...
    fun connect()
    {
        ...
        bootstrap.group(group).channel(NioSocketChannel::class.java).handler(object : ChannelInitializer<SocketChannel>()
        {
    ...
    {
        val gson = GsonBuilder().registerTypeAdapter(Packet::class.java, deserializer).create()
    ...
    {
        val gson = GsonBuilder().registerTypeAdapter(Packet::class.java, serializer).create()
