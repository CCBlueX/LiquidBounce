...
object AntiBot : Module()
{
	...
	{ // Check if entity is a player
		...
		if (entityIDValue.get() && (entity.entityId >= 1000000000 || entity.entityId <= -1)) return true

		if (derpValue.get() && (entity.rotationPitch > 90F || entity.rotationPitch < -90F)) return true
		...
		if (pingValue.get())
		{
			if (mc.netHandler.getPlayerInfo(entity.asEntityPlayer().uniqueID)?.responseTime == 0) return true
		}
		...
		if (duplicateInTabValue.get() && mc.netHandler.playerInfoMap.filter { entity.name == stripColor(it.getFullName()) }.count() > 1) return true

		if (allwaysInRadiusValue.get() && !notAlwaysInRadius.contains(entity.entityId)) return true
	...
	fun onPacket(event: PacketEvent)
	{
		...
		if (classProvider.isSPacketEntity(packet))
		{
			...
			if (classProvider.isEntityPlayer(entity) && entity != null)
			{
				...
				if (!notAlwaysInRadius.contains(entity.entityId) && mc.thePlayer!!.getDistanceToEntity(entity) > allwaysRadiusValue.get()) notAlwaysInRadius.add(entity.entityId)
			}
		}
