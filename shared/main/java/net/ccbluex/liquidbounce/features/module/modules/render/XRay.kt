...
class XRay : Module()
{
	...
	fun canBeRendered(pos: WBlockPos, block: IBlock): Boolean
	{
		val theWorld = mc.theWorld ?: return false

		return xrayBlocks.contains(block) && (!orbfuscatorBypass || facings.map { pos.offset(it) }.any { !BlockUtils.isFullBlock(theWorld, it) })
	}
