...
public final class RotationUtils extends MinecraftInstance implements Listenable
{
		...
		for (double xSearch = 0.1D; xSearch < 0.9D; xSearch += 0.1D)
			for (double ySearch = 0.1D; ySearch < 0.9D; ySearch += 0.1D)
				for (double zSearch = 0.1D; zSearch < 0.9D; zSearch += 0.1D) {
					...
					final Rotation rotation = new Rotation(WMathHelper.wrapAngleTo180_float((float) Math.toDegrees(Math.atan2(diffZ, diffX)) - 90F), WMathHelper.wrapAngleTo180_float((float) -Math.toDegrees(Math.atan2(diffY, diffXZ))));
	 ...
	 * @param predict
	 *                    predict new enemy position
	 * @param predictSize
	 *                    predict size of predict
	 */
	public static void faceBow(final IEntity target, final boolean silent, final boolean predict, final float predictSize)
	{
		...
		final Rotation rotation = new Rotation((float) (Math.atan2(posZ, posX) * 180 / Math.PI) - 90, (float) -Math.toDegrees(Math.atan((velocity * velocity - Math.sqrt(velocity * velocity * velocity * velocity - 0.006F * (0.006F * (posSqrt * posSqrt) + 2 * posY * (velocity * velocity)))) / (0.006F * posSqrt))));

		if (silent)
			setTargetRotation(rotation);
		else
			limitAngleChange(new Rotation(player.getRotationYaw(), player.getRotationPitch()), rotation, 10 + new Random().nextInt(6)).toPlayer(mc.getThePlayer());
	}
	...
	public static Rotation toRotation(final WVec3 vec, final boolean predict)
	{
		...
		return new Rotation(WMathHelper.wrapAngleTo180_float((float) Math.toDegrees(Math.atan2(diffZ, diffX)) - 90F), WMathHelper.wrapAngleTo180_float((float) -Math.toDegrees(Math.atan2(diffY, Math.sqrt(diffX * diffX + diffZ * diffZ)))));
	}
