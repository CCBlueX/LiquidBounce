...
public final class RotationUtils extends MinecraftInstance implements Listenable
{
	...
	public static void faceBow(final IEntity target, final boolean silent, final boolean enemyPrediction, final boolean playerPrediction, final float minTurnSpeed, final float maxTurnSpeed, final float minSmoothingRatio, final float maxSmoothingRatio)
	{
		...
		// Calculate Rotation
		final Rotation rotation = new Rotation((float) (atan2(posZ, posX) * 180 / PI) - 90, (float) -toDegrees(atan((velocity * velocity - sqrt(velocity * velocity * velocity * velocity - 0.006F * (0.006F * (posSqrt * posSqrt) + 2 * posY * (velocity * velocity)))) / (0.006F * posSqrt))));
	...
	{
		return hypot(getAngleDifference(rot1.getYaw(), rot2.getYaw()), getAngleDifference(rot1.getPitch(), rot2.getPitch()));
	}
