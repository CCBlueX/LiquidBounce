...
object LoginUtils : MinecraftInstance()
{
	...
	fun login(serviceType: MinecraftAccount.AltServiceType, username: String, password: String?): LoginResult
	{
		...
		return try
		{
			...
			mc.session = classProvider.createSession(userAuthentication.selectedProfile.name, userAuthentication.selectedProfile.id.toString(), userAuthentication.authenticatedToken, "mojang")
	...
	{
		INVALID_ACCOUNT_DATA, AUTHENTICATION_FAILURE, AUTHENTICATION_UNAVAILABLE, MIGRATED, MCLEAKS_INVALID, THEALTENING_INVALID, FAILED_PARSE_SESSION, LOGGED_IN
	}
}
