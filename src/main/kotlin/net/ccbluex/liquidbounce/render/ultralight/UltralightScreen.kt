...
import net.ccbluex.liquidbounce.render.ultralight.bindings.QueuedScreen
...
object UltralightScreenHook : Listenable {
    ...
    val screenHandler = handler<ScreenEvent> { event ->
        ...
        event.cancelEvent()
        mc.openScreen(UltralightScreen(page))
    }

}

class UltralightScreen(val page: Page) : Screen(page.name.asText()) {

    private lateinit var webView: WebView

    override fun init() {
        webView = WebView(width = { mc.window.width }, height = { mc.window.height }, screen = this)
