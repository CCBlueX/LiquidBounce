...
object CommandExecutor : EventListener {
            ...
            is CommandException -> {
                ...
                if (e.usageInfo.isNotEmpty()) {
                    ...
                    for (usage in e.usageInfo) {
                        val prefix = CommandManager.Settings.prefix
                        val text = regular("")
    ...
    private val chatEventHandler = handler<ChatSendEvent>(priority = EventPriorityConvention.FIRST_PRIORITY) {
        if (!it.message.startsWith(CommandManager.Settings.prefix)) {
            return@handler
        }

        val commandBody = it.message.substring(CommandManager.Settings.prefix.length)
        try {
