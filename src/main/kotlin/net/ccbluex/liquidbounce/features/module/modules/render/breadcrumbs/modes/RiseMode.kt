...
object RiseMode : BreadcrumbsMode("Point") {
    ...
    val renderHandler = handler<WorldRenderEvent> { event ->
        ...
        renderEnvironmentForWorld(matrixStack) {
            ...
            path.forEach { point ->
                ...
                drawCircle(
                    Matrix4f(matrixStack.peek().positionMatrix),
                    0f, 0f, 0f, 0.7f,
                ...
                if (distance < 4) {
                    drawCircle(
                        Matrix4f(matrixStack.peek().positionMatrix),
                        0f, 0f, 0f, 1.4f,
                        color.withAlpha((alpha * 0.5f).toInt()),
                        quality
                    )
                }
                if (distance < 20) {
                    drawCircle(
                        Matrix4f(matrixStack.peek().positionMatrix),
                        0f, 0f, 0f, 2.3f,
                        color.withAlpha((alpha * 0.3f).toInt()),
                        quality
                    )
                }
                matrixStack.pop()
    ...
    private fun drawCircle(
        ...
        val step = (2.0 * Math.PI / quality)
        for (i in 0..quality) {
            val angle = i * step
            val vx = x + (cos(angle) * radius).toFloat()
            val vy = y + (sin(angle) * radius).toFloat()
            val vz = z
            buffer.vertex(matrix, vx, vy, vz).color(argb)
        }
