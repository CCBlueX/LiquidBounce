...
object Tabs {
     ...
     */
    private fun setupSpecial() = LiquidsItemGroup("Special", icon = {
     ...
     */
    private fun setupExploits() = LiquidsItemGroup("Exploits", icon = { ItemStack(Items.LINGERING_POTION) }, items = {
        // TODO: Add exploits
        ...
        it.add(
            PotionUtil.setCustomPotionEffects(
                ItemStack(Items.SPLASH_POTION).setCustomName("".asText().styled { s -> s.withBold(true) }
                    .append("Troll".asText().styled { s -> s.withColor(Formatting.RED) })
                    .append("Potion".asText().styled { s -> s.withColor(Formatting.GOLD) })
        ...
        it.add(
            PotionUtil.setCustomPotionEffects(
                ItemStack(Items.SPLASH_POTION).setCustomName("".asText().styled { s -> s.withBold(true) }
                    .append("Kill".asText().styled { s -> s.withColor(Formatting.RED) })
                    .append("Potion".asText().styled { s -> s.withColor(Formatting.GOLD) })
                ), listOf(
    ...
    private var headsDb = runCatching {
        ...
        if (headService.enabled) {
            ...
            heads.map { it.value }.toTypedArray().also {
    ...
    private fun setupHeads() = LiquidsItemGroup("Heads", icon = { ItemStack(Items.SKELETON_SKULL) }, items = {
        it.addAll(headsDb.distinctBy { it.name }.map { head ->
...
open class LiquidsItemGroup(
    val plainName: String, val icon: () -> ItemStack, val items: (items: ItemGroup.Entries) -> Unit
) {

    // Create item group and assign to minecraft groups
    fun create(): ItemGroup {
        // Expand array
        val itemGroup = ItemGroup.create(ItemGroup.Row.TOP, -1).displayName(plainName.asText()).icon(icon)
