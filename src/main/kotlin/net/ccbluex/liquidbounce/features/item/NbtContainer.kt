...
fun SimpleInventory.inventoryAsCompound(title: Text): List<NbtCompound> {
    ...
    stacks.chunked(27).forEachIndexed { containerIndex, stacks ->
        ...
        loreList.add("Container #$containerIndex".asText().withColor(Formatting.GOLD).asNbt(world))
        mc.currentServerEntry?.let {
            loreList.add("Server: ${it.address}".asText().withColor(Formatting.GOLD).asNbt(world))
        }
        loreList.add("".asText().asNbt(world))

        loreList.add(
            "Generated by ${LiquidBounce.CLIENT_NAME} ${LiquidBounce.clientVersion}".asText()
                .withColor(Formatting.DARK_AQUA)
                .asNbt(world)
        )
        loreList.add(
            "Made with <3 by ${LiquidBounce.CLIENT_AUTHOR}".asText()
