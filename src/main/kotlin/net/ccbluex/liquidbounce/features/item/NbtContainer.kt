...
fun SimpleInventory.inventoryAsCompound(title: Text): List<NbtCompound> {
    ...
    stacks.chunked(27).forEachIndexed { containerIndex, stacks ->
        ...
        loreList.add("Container #$containerIndex".asPlainText(Formatting.GOLD).asNbt(world))
        mc.currentServerEntry?.let {
            loreList.add("Server: ${it.address}".asPlainText(Formatting.GOLD).asNbt(world))
        }
        loreList.add("".asPlainText().asNbt(world))
