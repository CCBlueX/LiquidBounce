...
class MineplexFly : FlyMode("Mineplex")
{
    ...
    override fun onBlockBB(event: BlockBBEvent)
    {
        ...
        if (event.block is BlockAir && canFly(thePlayer) && event.y < thePlayer.posY) event.boundingBox = AxisAlignedBB(event.x.toDouble(), event.y.toDouble(), event.z.toDouble(), event.x + 1.0, thePlayer.posY, event.z + 1.0)
    }
