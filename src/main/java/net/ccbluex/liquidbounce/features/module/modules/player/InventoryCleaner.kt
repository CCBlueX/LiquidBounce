...
object InventoryCleaner: Module("InventoryCleaner", Category.PLAYER, hideModule = false) {
	...
	private fun isUsefulLighter(stack: ItemStack?, stacks: List<ItemStack?>, entityStacksMap: Map<ItemStack, EntityItem>? = null): Boolean {
		...
		if (!entityStacksMap.isNullOrEmpty()) {
			distanceSqToItem = player.getDistanceSqToEntity(entityStacksMap[stack] ?: return false)
		...
		return stacksToIterate.withIndex().none { (otherIndex, otherStack) ->
			...
			if (index == otherIndex) {
				...
				return distanceSqToItem > player.getDistanceSqToEntity(otherEntityItem)
			}
			...
			when (otherStackSaturation.compareTo(stackSaturation)) {
				...
				0 -> {
					...
					if (index == otherIndex) {
						...
						distanceSqToItem > player.getDistanceSqToEntity(otherEntityItem)
					} else {
					...
					0 -> when (otherStack.totalDurability.compareTo(stack.totalDurability)) {
						...
						0 ->  {
							...
							if (index == otherIndex) {
								...
								when (distanceSqToItem.compareTo(player.getDistanceSqToEntity(otherEntityItem))) {
									1 -> return false
