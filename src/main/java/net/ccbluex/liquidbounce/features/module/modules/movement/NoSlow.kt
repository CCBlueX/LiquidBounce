    ...
    fun onMotion(event: MotionEvent)
    {
        ...
        if (packetEnabledValue.get() && Backend.MINECRAFT_VERSION_MINOR == 8)
        {
            ...
            if (ncpDelay.hasTimePassed(packetDelayValue.get()))
            {
                ...
                {
                    EventState.PRE -> if (packetUnblock.get()) netHandler.addToSendQueue(CPacketPlayerDigging(ICPacketPlayerDigging.WAction.RELEASE_USE_ITEM, BlockPos(0, 0, 0), provider.getEnumFacing(EnumFacingType.DOWN))) // Un-block
                    EventState.POST -> if (packetBlockValue.get()) netHandler.addToSendQueue(CPacketPlayerBlockPlacement(BlockPos(-1, -1, -1), 255, thePlayer.inventory.getCurrentItemInHand(), 0.0F, 0.0F, 0.0F)) // (Re-)Block
                }
