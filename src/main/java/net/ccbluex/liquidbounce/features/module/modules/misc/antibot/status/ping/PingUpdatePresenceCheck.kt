...
class PingUpdatePresenceCheck : BotCheck("status.ping.updatePresence")
{
    ...
    override fun onPacket(event: PacketEvent)
    {
        ...
        if (packet is S38PacketPlayerListItem)
        {
            ...
            when (packet.action)
            {
                S38PacketPlayerListItem.Action.UPDATE_LATENCY -> latencyUpdated(updatedPlayers, playerInfoMap)
    ...
    private fun latencyUpdated(updatedPlayers: MutableList<S38PacketPlayerListItem.AddPlayerData>, playerInfoMap: MutableCollection<NetworkPlayerInfo>)
    {
