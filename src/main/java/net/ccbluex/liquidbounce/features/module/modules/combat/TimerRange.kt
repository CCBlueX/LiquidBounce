...
object TimerRange : Module("TimerRange", Category.COMBAT, hideModule = false) {
    ...
    fun onAttack(event: AttackEvent) {
        ...
        if (shouldReturn || (player.isInWeb && !onWeb) || (player.isInWater && !onWater)) {
            return
        }
    ...
    private fun isPlayerMoving(): Boolean {
        return !mc.gameSettings.keyBindBack.isKeyDown && (player?.moveForward != 0f || player?.moveStrafing != 0f)
    }
