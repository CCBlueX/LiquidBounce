...
class AutoBow : Module()
{
    ...
    fun onUpdate(@Suppress("UNUSED_PARAMETER") event: UpdateEvent)
    {
        ...
        if (thePlayer.isUsingItem && thePlayer.heldItem?.item is ItemBow && thePlayer.itemInUseDuration > 20 && (!waitForBowAimbot.get() || !bowAimbot.state || bowAimbot.hasTarget(thePlayer)))
        {
            thePlayer.stopUsingItem()
            mc.netHandler.addToSendQueue(CPacketPlayerDigging(ICPacketPlayerDigging.WAction.RELEASE_USE_ITEM, BlockPos.ORIGIN, classProvider.getEnumFacing(EnumFacingType.DOWN)))
        }
    }
}
