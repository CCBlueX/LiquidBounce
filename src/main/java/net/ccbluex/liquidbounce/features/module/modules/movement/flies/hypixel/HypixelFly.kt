...
class HypixelFly : FlyMode("Hypixel")
{
    ...
    override fun onBlockBB(event: BlockBBEvent)
    {
        ...
        if (event.block is BlockAir && hypixelFlyStarted && event.y < posY) event.boundingBox = AxisAlignedBB(event.x.toDouble(), event.y.toDouble(), event.z.toDouble(), event.x + 1.0, posY, event.z + 1.0)
    }
