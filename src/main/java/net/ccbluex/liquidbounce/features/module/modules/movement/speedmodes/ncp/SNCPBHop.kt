...
object SNCPBHop : SpeedMode("SNCPBHop") {
    ...
    override fun onMove(event: MoveEvent) {
        ...
        if (level == 1 && isMoving) {
            ...
            if (mc.theWorld.getCollidingBoundingBoxes(player, player.entityBoundingBox.offset(0.0, player.motionY, 0.0)).isNotEmpty() || player.isCollidedVertically) level = 1
            lastDist = 0.0
            moveSpeed = baseMoveSpeed
            return
        } else {
            if (mc.theWorld.getCollidingBoundingBoxes(player, player.entityBoundingBox.offset(0.0, player.motionY, 0.0)).isNotEmpty() || player.isCollidedVertically) {
                moveSpeed = baseMoveSpeed
                lastDist = 0.0
                level = 88
                return
    ...
    private val baseMoveSpeed: Double
        get() {
            ...
            if (player.isPotionActive(Potion.moveSpeed)) baseSpeed *= 1.0 + 0.2 * (player.getActivePotionEffect(Potion.moveSpeed).amplifier + 1)
            return baseSpeed
        }
