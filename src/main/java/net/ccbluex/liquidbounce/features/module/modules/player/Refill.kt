...
object Refill : Module("Refill", Category.PLAYER, hideModule = false) {
    ...
    fun onTick(event: TickEvent) {
        ...
        for (slot in 36..44) {
            val stack = player.inventoryContainer.getSlot(slot).stack ?: continue
            if (stack.stackSize == stack.maxStackSize || !stack.hasItemAgePassed(minItemAge)) continue

            when (mode) {
                "Swap" -> {
                    val bestOption = player.inventoryContainer.inventory.withIndex()
    ...
    fun click(slot: Int, button: Int, mode: Int, stack: ItemStack) {
        ...
        sendPacket(
            C0EPacketClickWindow(player.openContainer.windowId, slot, button, mode, stack,
