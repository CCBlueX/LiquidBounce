...
object UpdateInfo
{
    ...
    val newestVersion by lazy {
        ...
        try
        {
            Gson().fromJson(HttpUtils["$CLIENT_API/version/newest/${gitInfo["git.branch"]}${if (LiquidBounce.IN_DEV) "" else "/release"}"], Build::class.java)
        }
        catch (e: Exception)
        {
            ClientUtils.logger.error("Unable to receive update information", e)
            return@lazy null
        }
    }
    ...
    fun hasUpdate(): Boolean
    {
        try
        {
            ...
            return if (LiquidBounce.IN_DEV)
            { // check if new build is newer than current build
        ...
        {
            ClientUtils.logger.error("Unable to check for update", e)
            return false
        }
    }
}

data class Build(@SerializedName("build_id") val buildId: Int, @SerializedName("commit_id") val commitId: String, val branch: String, @SerializedName("lb_version") val lbVersion: String, @SerializedName("mc_version") val mcVersion: String, val release: Boolean, val date: String, val message: String, val url: String)
