...
class Script(val scriptFile: File) : MinecraftInstance()
{
    ...
    init
    {
        val engineFlags = getMagicComment("engine_flags")?.split(",")?.map(String::trim)?.toTypedArray() ?: emptyArray()
    ...
    private fun supportLegacyScripts()
    {
        if (getMagicComment("api_version") != "2")
        {
            ...
            LiquidBounce::class.java.getResource("/assets/minecraft/liquidbounce/scriptapi/legacy.js")?.readText()?.let<String, Any?>(scriptEngine::eval)
        }
    }
