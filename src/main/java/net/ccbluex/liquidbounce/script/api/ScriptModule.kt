...
class ScriptModule(private val moduleObject: JSObject) : Module()
{
    ...
    init
    {
        ...
        ModuleCategory.values().filter { it.displayName.equals(categoryString, true) }.forEach { this.category = it }

        if (moduleObject.hasMember("settings"))
        {
            val settings = moduleObject.getMember("settings") as JSObject

            for (settingName in settings.keySet()) _values[settingName] = settings.getMember(settingName) as Value<*>
        }

        if (moduleObject.hasMember("tag")) _tag = moduleObject.getMember("tag") as String
    }
