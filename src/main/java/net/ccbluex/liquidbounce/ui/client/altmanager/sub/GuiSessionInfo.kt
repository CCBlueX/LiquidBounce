...
class GuiSessionInfo(private val prevGui: GuiScreen, private val defaultSessionId: String?) : GuiScreen()
{
    ...
    private lateinit var decodeButton: GuiButton
    ...
    override fun initGui()
    {
        ...
        buttonList.add(GuiButton(1, buttonX, buttonY - 48, "Analyze").also { decodeButton = it })
        buttonList.add(GuiButton(2, buttonX, buttonY - 24, "Clipboard").also { clipboardButton = it })
        buttonList.add(GuiButton(3, buttonX, buttonY - 72, "Login").also { loginButton = it })
        buttonList.add(GuiButton(0, buttonX, buttonY, "Back"))

        val token = if (defaultSessionId != null && defaultSessionId.isNotEmpty()) defaultSessionId else mc.session.token

        sessionIdField = GuiTextField(2, Fonts.font40, middleScreen - 300, 60, 600, 20).apply {
            isFocused = true
    ...
    override fun actionPerformed(button: GuiButton)
    {
        ...
        when (button.id)
        {
            ...
            3 ->
            {
                ...
                val sl = GuiSessionLogin(this)

                mc.displayGuiScreen(sl)
