...
class GuiLoginIntoAccount(private val prevGui: GuiAltManager, private val directLogin: Boolean = false) : GuiScreen()
{
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float)
    {
        ...
        {
            Fonts.font40.drawCenteredString("\u00A77Username / E-Mail", (width / 2 - 55).toFloat(), 66f, 0xffffff)
        }

        if (password.text.isEmpty() && !password.isFocused)
        {
            Fonts.font40.drawCenteredString("\u00A77Password", (width / 2 - 74).toFloat(), 91f, 0xffffff)
        }
        ...
        when (button.id)
        {
            ...
            3 ->
            {
                MicrosoftAccount.buildFromOpenBrowser(object : MicrosoftAccount.OAuthHandler
                {
                    ...
                    {
                        if (LiquidBounce.fileManager.accountsConfig.isAccountExists(account))
                        {
    ...
    private fun checkAndAddAccount(usernameText: String, passwordText: String)
    {
        ...
        {
            // cracked account
            CrackedAccount().also { it.name = usernameText }
        }
        else
        {
            // mojang account
            MojangAccount().also {
                it.email = usernameText
                it.password = passwordText
            }
        }

        if (LiquidBounce.fileManager.accountsConfig.isAccountExists(minecraftAccount))
        {
            status = "\u00A7cThe account has already been added."
            return
        }
