...
class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F, side: Side = Side.default()) : Element(x, y, scale, side)
{
    ...
    override fun drawElement(): Border
    {
        ...
        val borderYEnd = fontRenderer.FONT_HEIGHT.toFloat() + borderExpand
        ...
        RainbowFontShader.begin(textRainbowShader, rainbowShaderX, rainbowShaderY, rainbowShaderOffset).use {
            ...
            if (editMode && mc.currentScreen is GuiHudDesigner && editTicks <= 10) fontRenderer.drawString("_", (if (rightRect) 0f else if (leftRect) rectWidth else rectWidth * 0.5F) + (borderXStart + borderExpand) + fontRenderer.getStringWidth(displayText.take(cursor)) + 2F, 0F, textColor, shadow)
        }
