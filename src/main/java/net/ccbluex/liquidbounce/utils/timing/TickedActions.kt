...
object TickedActions : Listenable {
    ...
    class TickScheduler(val module: Module) : MinecraftInstance() {
        ...
        fun scheduleClick(slot: Int, button: Int, mode: Int, allowDuplicates: Boolean = false, windowId: Int = player.openContainer.windowId, action: ((ItemStack?) -> Unit)? = null) =
            schedule(slot, module, allowDuplicates) {
                val newStack = mc.playerController.windowClick(windowId, slot, button, mode, player)
