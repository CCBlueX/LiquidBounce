...
object TickedActions : Listenable {
    ...
    class TickScheduler(val module: Module) : MinecraftInstance() {
        ...
        fun scheduleClick(slot: Int, button: Int, mode: Int, allowDuplicates: Boolean = false, windowId: Int = mc.player.playerScreenHandler.syncId, action: ((ItemStack?) -> Unit)? = null) =
            schedule(slot, module, allowDuplicates) {
                val newStack = mc.interactionManager.clickSlot(windowId, slot, button, mode, mc.player)
