...
class ArmorComparator : MinecraftInstance(), Comparator<ArmorPiece>, Serializable
{
    override fun compare(armor: ArmorPiece, otherArmor: ArmorPiece): Int
    {
        ...
        if (compare == 0)
        {
            ...
            if (otherEnchantmentCmp == 0)
            {
                ...
                // Then durability...
                val o1a = (stack.item as ItemArmor?) ?: return -1
                val o2a = (otherStack.item as ItemArmor?) ?: return 1
    ...
    companion object
    {
        ...
        private fun getThresholdedEnchantmentDamageReduction(itemStack: ItemStack): Float = DAMAGE_REDUCTION_ENCHANTMENTS.indices.map { itemStack.getEnchantmentLevel(DAMAGE_REDUCTION_ENCHANTMENTS[it]) * ENCHANTMENT_FACTORS[it] * ENCHANTMENT_DAMAGE_REDUCTION_FACTOR[it] }.sum()

        private fun getEnchantmentThreshold(itemStack: ItemStack): Float = OTHER_ENCHANTMENTS.indices.map { itemStack.getEnchantmentLevel(OTHER_ENCHANTMENTS[it]) * OTHER_ENCHANTMENT_FACTORS[it] }.sum()
