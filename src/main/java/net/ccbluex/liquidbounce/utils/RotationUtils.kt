...
class RotationUtils : MinecraftInstance(), Listenable
{
    ...
    fun onPacket(event: PacketEvent)
    {
        ...
        if (packet is C03PacketPlayer)
        {
            ...
            {
                packet.yaw = targetRot.yaw
                packet.pitch = targetRot.pitch
    ...
    companion object
    {
         ...
         * target block
         */
        fun faceBlock(theWorld: World, thePlayer: Entity, blockPos: BlockPos): VecRotation?
        {
            ...
            while (xSearch < 0.9)
            {
                ...
                while (ySearch < 0.9)
                {
                    ...
                    while (zSearch < 0.9)
                    {
                        val posVec = Vec3(blockPos).plus(xSearch, ySearch, zSearch)
                        ...
                        val rotation = Rotation(wrapAngleTo180_float((atan2(diffZ, diffX).toFloat()).toDegrees - 90.0f), wrapAngleTo180_float(-(atan2(diffY, diffXZ).toFloat()).toDegrees))
                        val rotationVector = getVectorForRotation(rotation)
                        val vector = eyesPos + rotationVector * dist
                        val obj = theWorld.rayTraceBlocks(eyesPos, vector, false, false, true)
         ...
         */
        fun faceBow(thePlayer: EntityPlayer, target: Entity, turnSpeed: MinMaxPair, smoothingRatio: MinMaxPair, playerPredictSize: MinMaxPair, flags: Int)
        {
            ...
            val sprinting = target.isSprinting
            ...
            val rotation = Rotation((atan2(posZ, posX).toFloat()).toDegrees - 90, -(atan((velocitySq - sqrt(velocity.pow(4) - 0.006f * (0.006f * distSq + 2 * posY * velocitySq))) / (0.006f * dist)).toFloat()).toDegrees)
         ...
         * @return               rotation
         */
        fun toRotation(thePlayer: Entity, vec: Vec3, playerPredict: Boolean, playerPredictSize: MinMaxPair): Rotation
        {
            val posX = thePlayer.posX
            val posZ = thePlayer.posZ
            var eyesPos = Vec3(posX, thePlayer.entityBoundingBox.minY + thePlayer.eyeHeight, posZ)
            ...
            return Rotation(wrapAngleTo180_float((atan2(diffZ, diffX).toFloat()).toDegrees - 90.0f), wrapAngleTo180_float(-(atan2(diffY, hypot(diffX, diffZ)).toFloat()).toDegrees))
        }
         ...
         */
        fun getCenter(box: AxisAlignedBB): Vec3 = Vec3(box.minX + (box.maxX - box.minX) * 0.5, box.minY + (box.maxY - box.minY) * 0.5, box.minZ + (box.maxZ - box.minZ) * 0.5)
         ...
         * @return                   center
         */
        fun searchCenter(theWorld: World, thePlayer: Entity, targetBox: AxisAlignedBB, flags: Int, jitterData: JitterData?, playerPredictSize: MinMaxPair, distance: Float, boxShrink: Double, steps: Int, randomCenterSize: Double, distanceOutOfRangeCallback: (() -> Unit)? = null): VecRotation?
        {
            val randomVec: Vec3
            ...
            when
            {
                ...
                {
                    randomVec = Vec3(minX + (maxX - minX) * (x * 0.3 + 1.0), minY + (maxY - minY) * (y * 0.3 + 1.0), minZ + (maxZ - minZ) * (z * 0.3 + 1.0))
                    return VecRotation(randomVec, toRotation(thePlayer, randomVec, playerPredict, playerPredictSize))
                }
            ...
            randomVec = Vec3(minX + (maxX - minX) * x * randomCenterSize, minY + (maxY - minY) * y * randomCenterSize, minZ + (maxZ - minZ) * z * randomCenterSize)
            ...
            while (xSearch < boxEnd)
            {
                ...
                while (ySearch < boxEnd)
                {
                    ...
                    {
                        val vec3 = Vec3(minX + (maxX - minX) * xSearch, minY + (maxY - minY) * ySearch, minZ + (maxZ - minZ) * zSearch)
         ...
         */
        fun getClientRotationDifference(thePlayer: Entity, entity: Entity, playerPredict: Boolean, playerPredictSize: MinMaxPair): Double = getRotationDifference(toRotation(thePlayer, getCenter(entity.entityBoundingBox), playerPredict, playerPredictSize), clientRotation)
         ...
         * @return        difference between rotation
         */
        fun getClientRotationDifference(thePlayer: Entity, blockPos: BlockPos, playerPredict: Boolean, playerPredictSize: MinMaxPair): Double = getRotationDifference(toRotation(thePlayer, Vec3(blockPos.x + 0.5, blockPos.y + 0.5, blockPos.z + 0.5), playerPredict, playerPredictSize), clientRotation)
         ...
         * @return        difference between rotation
         */
        fun getServerRotationDifference(thePlayer: Entity, entity: Entity, playerPredict: Boolean, playerPredictSize: MinMaxPair): Double = getServerRotationDifference(thePlayer, getCenter(entity.entityBoundingBox), playerPredict, playerPredictSize)
         ...
         * @return        difference between rotation
         */
        fun getServerRotationDifference(thePlayer: Entity, blockPos: BlockPos, playerPredict: Boolean, playerPredictSize: MinMaxPair): Double = getServerRotationDifference(thePlayer, Vec3(blockPos.x + 0.5, blockPos.y + 0.5, blockPos.z + 0.5), playerPredict, playerPredictSize)

        private fun getServerRotationDifference(thePlayer: Entity, pos: Vec3, playerPredict: Boolean, playerPredictSize: MinMaxPair): Double = getRotationDifference(toRotation(thePlayer, pos, playerPredict, playerPredictSize), serverRotation)
        ...
        //  * @return        difference between rotation
        //  */
        // fun getRotationDifference(thePlayer: Entity, entity: Entity): Double
        // {
        ...
        fun getVectorForRotation(rotation: Rotation): Vec3
        {
            ...
            val pitchCos = -(-pitchRadians).cos.toDouble()
            return Vec3((-yawRadians - PI).sin * pitchCos, (-pitchRadians).sin.toDouble(), (-yawRadians - PI).cos * pitchCos)
        }
         ...
         */
        fun isFaced(theWorld: World, thePlayer: Entity, targetEntity: Entity?, reachDistance: Double, expandRange: Double = 1.0, aabbGetter: (Entity) -> AxisAlignedBB = Entity::getEntityBoundingBox): Boolean
        {
         ...
         */
        fun isVisible(theWorld: World, thePlayer: Entity, vec3: Vec3): Boolean = theWorld.rayTraceBlocks(Vec3(thePlayer.posX, thePlayer.entityBoundingBox.minY + thePlayer.eyeHeight, thePlayer.posZ), vec3) == null
